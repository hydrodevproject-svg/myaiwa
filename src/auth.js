/* ==========================================================================
   MYAIWA - AUTHENTICATION, USER PROFILE & ACCESS CONTROL (REALTIME SYNC)
   ========================================================================== */
...
// ==========================================
// 1. SINKRONISASI REALTIME PARAMETER ROLE
// ==========================================
export function initRealtimeSystemParameters() {
  if (unsubscribeRoleParamsSnapshot) unsubscribeRoleParamsSnapshot();

  unsubscribeRoleParamsSnapshot = onSnapshot(doc(db, "app_settings", "parameters_roles"), (docSnap) => {
    if (docSnap.exists()) {
      state.roleParamsCache = { ...state.roleParamsCache, ...docSnap.data() };
      updateAllShiftCardsTimeDisplay();
      if (auth.currentUser && window.calculateUserKPI) {
        window.calculateUserKPI(auth.currentUser.uid);
      }
    }
  }, (err) => {
    console.warn("Realtime params listener error:", err);
  });
}
...
// ==========================================
// 2. AUTH STATE OBSERVER & REALTIME PROFILE
// ==========================================
    ...
    if (user) {
      ...
      unsubscribeUserSnapshot = onSnapshot(doc(db, "users", user.uid), async (userDoc) => {
        if (userDoc.exists()) {
          ...
          const inputName = document.getElementById("profile-full-name");
          const inputPhone = document.getElementById("profile-phone");
          const inputAddr = document.getElementById("profile-address");

          if (inputName && document.activeElement !== inputName) inputName.value = state.currentUserData.nama || "";
          if (inputPhone && document.activeElement !== inputPhone) inputPhone.value = state.currentUserData.phone || "";
          if (inputAddr && document.activeElement !== inputAddr) inputAddr.value = state.currentUserData.alamat || "";
...
export function renderRoleQuickActions(role) {
  ...
  const cardCuti = `
    <div class="hr-icon-card" onclick="openLeaveFormPage('Izin')">
