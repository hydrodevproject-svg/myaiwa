/* ==========================================================================
   MYAIWA - GPS ATTENDANCE, ROSTER RESOLVER & EARLY CHECK-OUT ENGINE
   ========================================================================== */
...
export function initMapLibre() {
  ...
  if (state.maplibreMap) { state.maplibreMap.resize(); return; }
...
export function refreshMapLibreGPS() { getGPSLocation(false); }

// ==========================================
// EKSEKUSI PRESENSI & DETEKSI PULANG AWAL
// ==========================================
export async function executeGPSAttendance() {
  ...
  try {
    ...
    if (!existingSnap.exists()) {
      ...
      // Deteksi Jam Pulang Shift Resmi
      ...
      // Jika waktu saat ini < jam pulang resmi -> Tampilkan Interceptor Konfirmasi Pulang Awal
      if (timeStr < officialEndTime) {
        ...
        if (confirmEarly) {
          ...
          return; // Batalkan check-out jika user memilih tidak
        }
      }

      // Check-out Normal / Setelah Jam Kerja Selesai
...
// ==========================================
// SUB-PAGE: PULANG AWAL (IZIN / SAKIT)
// ==========================================
